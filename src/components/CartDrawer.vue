...
<template>
    <div>
        ...
        ></transition>
        <div
            class="fixed top-0 right-0 h-full w-full md:w-[420px] bg-white dark:bg-charcoal z-[70] shadow-2xl transform transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1)"
            :class="store.isOpen ? 'translate-x-0' : 'translate-x-full'"
        >
            <!-- ... (Isi Template Sama) ... -->
            <div class="h-full flex flex-col relative overflow-hidden">
                <div
                    class="absolute top-0 right-0 w-64 h-64 bg-sky-100 rounded-full blur-3xl -z-10 opacity-60 pointer-events-none"
                ></div>
                <div
                    class="p-6 flex justify-between items-center bg-white/50 backdrop-blur-md border-b border-sky-100/50"
                >
                    <h2
                    ...
                    <button
                        @click="store.toggleCart()"
                        class="p-2 hover:bg-rose-50 text-slate-400 hover:text-rose-500 rounded-full transition"
                    >
                        <X :size="24" />
                    </button>
                </div>
                ...
                >
                    ...
                    >
                        <div
                            class="bg-sky-50 p-6 rounded-full mb-4 animate-float-slow"
                        >
                    ...
                    <div
                        ...
                        :key="item.id"
                        class="bg-white p-4 rounded-3xl border border-sky-50 shadow-sm flex gap-4 items-start group hover:shadow-md transition relative"
                    >
                        <div
                        ...
                        <div class="flex-1 min-w-0 pr-8">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div
                                    class="flex items-center gap-2 bg-slate-50 rounded-lg p-1 border border-slate-100"
                                >
                                    <button
                                        @click="store.updateQty(item.id, -1)"
                                        class="w-5 h-5 flex items-center justify-center text-slate-400 hover:bg-white hover:text-rose-500 rounded transition"
                                    >
                                        -</button
                                    ><span
                                        class="text-xs font-bold w-4 text-center text-slate-600"
                                        >{{ item.qty }}</span
                                    ><button
                                        @click="store.updateQty(item.id, 1)"
                                        class="w-5 h-5 flex items-center justify-center text-slate-400 hover:bg-white hover:text-emerald-500 rounded transition"
                                    >
                                        +
                                    </button>
                                </div>
                        ...
                        <button
                            ...
                            class="absolute top-3 right-3 p-1.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition"
                            title="Hapus Item"
                        >
                            <Trash2 :size="16" />
                        </button>
                ...
                <!-- Footer -->
                <div
                    class="bg-white border-t border-dashed border-sky-100 relative z-10 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]"
                >
                    <!-- Promo -->
                    <div class="px-6 pt-4 pb-2">
                        <div class="flex gap-2">
                            <div class="relative flex-1 group">
                                ...
                                <input
                                    ...
                                    class="pl-9 pr-8 w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 text-sm focus:outline-none focus:border-sky-300 focus:ring-4 focus:ring-sky-50 transition uppercase font-bold text-slate-600 placeholder:font-normal"
                            ...
                            <button
                                ...
                                "
                                class="bg-slate-800 text-white px-5 py-2 rounded-xl text-xs font-bold hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-lg shadow-slate-200 flex items-center justify-center min-w-[70px]"
                            >
                                <Loader2
                        ...
                        <div
                            ...
                            class="mt-2 flex items-start gap-2 bg-amber-50 p-2 rounded-lg border border-amber-100 text-[10px] text-amber-600"
                        >
                    ...
                    <div class="px-6 py-2">
                        ...
                        <transition name="fade">
                            <div
                                ...
                                class="mt-3 bg-indigo-50 p-3 rounded-xl border border-indigo-100 relative overflow-hidden"
                            >
                                <div v-if="userStore.user">
                                    ...
                                    >
                                        <span
                                            class="text-xs font-bold text-indigo-700"
                                            >Saldo Kamu:</span
                                        ><span
                                            class="text-sm font-extrabold text-indigo-600"
                                            >{{
                                                formatRupiah(
                                    ...
                                    >
                                        <UserCheck :size="12" /> Saldo cukup,
                                        siap potong!
                                    </div>
                                </div>
                                <div v-else>
                                    <p
                                        class="text-[10px] text-indigo-500 mb-2 font-bold text-center"
                                    >
                                        Login Member Lama 🔐
                                    </p>
                                    <div class="space-y-2">
                                        <input
                                            ...
                                            type="email"
                                            placeholder="Email"
                                            class="w-full px-3 py-1.5 text-xs rounded-lg border border-indigo-200 focus:outline-none focus:border-indigo-400"
                                        />
                                        <input
                    ...
                    <div class="p-6 pt-2 space-y-3">
                        ...
                        <div
                            v-if="promoStore.activeCode && isPromoEligible"
                            class="flex justify-between items-center text-xs font-bold text-emerald-500 bg-emerald-50 p-2 rounded-lg border border-emerald-100 animate-in slide-in-from-left-2"
                        >
                        ...
                        <div
                            class="flex justify-between items-end pt-2 border-t border-slate-100"
                        >
                            <span class="text-sm font-bold text-slate-600"
                                >Total Bayar</span
                            ><span
                                class="text-2xl font-extrabold text-sky-500"
                                >{{
                                    formatRupiah(
                        ...
                        <button
                            ...
                            class="w-full btn-bouncy bg-gradient-to-r from-emerald-500 to-teal-400 text-white font-bold py-4 rounded-2xl shadow-lg shadow-emerald-100 flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-4 hover:brightness-105"
                        >
